...
% Which organisations?
% 
get an IRI which derives from the Belgian national company register. The class CatalogRecord is not used in the Belgian extension.
...
\item \textit{DCAT-AP-NO (Norway):} In the Norwegian DCAT-AP extension, most of the changes took place in the class Dataset and Distribution. The extension provides additional subject, creator, and access right comment properties for the class Dataset. Additionally some properties have been added in order to describe relations between different datasets. DCAT-AP-NO also provides a new property dct:identifier for the class Agent. On the other hand, the property dcat:mediaType in the class Distribution was excluded, since it was considered to be the same as dct:format. 
...
In the context of mandatory properties, \textit{dct:identifier} is a property which was changed most often. Indeed, four extensions changed the property for the class Dataset, while two replace it for the class Agent and one changed it for the class Standard. Furthermore, for the class Dataset, \textit{dct:identifier} is an optional property, but all four extensions changed it to mandatory.
